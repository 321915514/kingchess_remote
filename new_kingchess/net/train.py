...
class TrainPipeline():
    def __init__(self, init_model=None):
        ...
        if init_model:
            try:
                ...
                logger.info('从头训练')
                self.policy_value_net = PolicyValueNet(model_file=CONFIG_TRAIN['pytorch_model_path'], device='cuda:0')
    ...
    def start_play_black_is_mcts(self, init_mcts_player, is_shown=0):
        ...
        while True:
            ...
            if game.player == Player.black:
                ...
                action = send_request('10.122.7.125', 65432, json_game)
            else:
                action = bot2.get_action(game)
    ...
    def policy_update(self):
        ...
        for i in range(self.epochs):
            accuracy, loss, policy_loss, value_loss, entropy = self.policy_value_net.train_step(
                state_batch,
                mcts_probs_batch,
                winner_batch,
                self.learn_rate * self.lr_multiplier)
            new_probs, new_v = self.policy_value_net.policy_value(state_batch)
            kl = np.mean(np.sum(old_probs * np.log(
                (old_probs + 1e-10) / (new_probs + 1e-10)),
                                axis=1)
                         )
    ...
    def policy_evaluate(self, n_games=5):
        """
        ...
        for i in range(n_games):
            ...
            if winner == Player.white:
                win_white_expert += 1
        # for i in range(n_games):
        #     winner = self.start_play(current_mcts_player, random)
        #     if winner == Player.black:
        #         win_black_random += 1
        # for i in range(n_games):
        #     winner = self.start_play(random, current_mcts_player)
        #     if winner == Player.white:
        #         win_white_random += 1
        win_rate_player_black = 1.0 * (win_black_expert) / n_games
        win_rate_player_white = 1.0 * (win_white_expert) / n_games
    ...
    def run(self):
        ...
        try:
            while True:
                ...
                if (self.iters + 1) % self.export_engine_freq == 0:
                    if os.path.exists(CONFIG_TRAIN['pytorch_current_model_path']):
                        export_engine(CONFIG_TRAIN['pytorch_current_model_path'],
                                      CONFIG_TRAIN['pytorch_current_onnx_path'],
                                      CONFIG_TRAIN['pytorch_current_engine_path'])
                ...
                if (self.iters + 1) % self.check_freq == 0:
                    ...
                    if win_ratio_white > self.best_win_ratio_white or win_ratio_black > self.best_win_ratio_black:
                        ...
                        self.policy_value_net.save_model(CONFIG_TRAIN['pytorch_current_model_path'])
                        self.policy_value_net.save_model(CONFIG_TRAIN['pytorch_best_model_path'])

                        if win_ratio_black > self.best_win_ratio_black:
                            logger.info("new best policy!!!!!!!!")
                            self.best_win_ratio_black = win_ratio_black
                            self.policy_value_net.save_model(f'./model_torch/best_policy_black_{win_ratio_black}.model')
                        if win_ratio_white > self.best_win_ratio_white:
                            self.best_win_ratio_white = win_ratio_white
                            self.policy_value_net.save_model(f'./model_torch/best_policy_white_{win_ratio_white}.model')

                        if (
                                self.best_win_ratio_black == 1.0 or self.best_win_ratio_white == 1.0) and self.pure_mcts_playout_num < 5000:
                            self.pure_mcts_playout_num += 1000
                            self.best_win_ratio_black = 0.0
                            self.best_win_ratio_white = 0.0

                    else:
                        if os.path.exists(CONFIG_TRAIN['pytorch_best_model_path']):
                            self.policy_value_net.policy_value_net.load_state_dict(
                                torch.load(CONFIG_TRAIN['pytorch_best_model_path']))
                            self.policy_value_net.save_model(CONFIG_TRAIN['pytorch_current_model_path'])
                            export_engine(CONFIG_TRAIN['pytorch_current_model_path'],
                                          CONFIG_TRAIN['pytorch_current_onnx_path'],
                                          CONFIG_TRAIN['pytorch_current_engine_path'])
                            ...
                            self.policy_value_net.policy_value_net.load_state_dict(
                                torch.load(CONFIG_TRAIN['pytorch_model_path']))
                            self.policy_value_net.save_model(CONFIG_TRAIN['pytorch_current_model_path'])
                            export_engine(CONFIG_TRAIN['pytorch_current_model_path'],
                                          CONFIG_TRAIN['pytorch_current_onnx_path'],
                                          CONFIG_TRAIN['pytorch_current_engine_path'])
...
if __name__ == '__main__':
    # s = time.time()
    training_pipeline = TrainPipeline(CONFIG_TRAIN['pytorch_current_model_path'])
