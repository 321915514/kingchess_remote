...
class Net_8(nn.Module):
    ...
    def __init__(self, num_channels=128, num_res_blocks=5):
        ...
        self.policy_fc = nn.Linear(16 * 9 * 9, 1125)
        # 价值头
        self.value_conv = nn.Conv2d(in_channels=num_channels, out_channels=8, kernel_size=(1, 1), stride=(1, 1),
                                    bias=False)
        self.value_bn = nn.BatchNorm2d(8)
        self.value_act1 = nn.ReLU()
        self.value_fc1 = nn.Linear(8 * 9 * 9, 128)
        # self.value_act2 = nn.ReLU()
        self.value_fc2 = nn.Linear(128, 1)
...
class PolicyValueNet():
    ...
    def __init__(self, model_file=None, use_gpu=True, device='cuda:7'):
        ...
        # if self.use_gpu:
        self.policy_value_net = Net_8().to(self.device)
        # else:
        #     self.policy_value_net = Net()
        self.optimizer = optim.AdamW(self.policy_value_net.parameters(), lr=1e-5, betas=(0.9, 0.999), eps=1e-8,
    ...
    def train_step(self, state_batch, mcts_probs, winner_batch, lr=0.001):
