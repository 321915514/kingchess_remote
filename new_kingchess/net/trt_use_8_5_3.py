...
class TRTEngine:
    ...
    def __init_engine(self) -> None:
        ...
        log.info("Reading engine from file {}".format(self.weight))
    ...
    def policy_value_fn(self, game: GameState):
        ...
        assert len(current_state) == self.num_inputs
        contiguous_inputs: List[ndarray] = [
            np.ascontiguousarray(i) for i in current_state
        ]
        ...
        # result = tuple(outputs) if len(outputs) > 1 else outputs[0]

        log_act_probs, value = outputs[0], outputs[1]

        # print(log_act_probs) # 是 softmax之后的
        # print(value)
        act_probs = np.exp(log_act_probs.astype('float32').flatten())

        invalid_actions, _ = game.legal_position()


        mask = invalid_actions
        masked_prob = act_probs*mask
        masked_prob[mask == 0] = float('-inf')

        masked_prob = torch.softmax(masked_prob, dim=1)
...
if __name__ == '__main__':
    engine = TRTEngine('./pytorch_muc.trt')
    game = GameState.new_game(5, 9)

    # current_state = np.ascontiguousarray(encoder_board(game).reshape(-1, 5, 9, 9)).astype(
    #     'float32')  # .reshape(-1, 3)
    # end
    # image = torch.from_numpy(current_state).to(torch.device('cuda:8'))

    result = engine.policy_value_fn(game)
